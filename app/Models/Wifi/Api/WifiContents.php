...
class WifiContents extends Model
{
   ...
   public static function addOrUpdateWifiContentsInfo( $data = [] , $contentid = null )
   {
      ...
      if ( intval ( $contentid ) )
      {
         ...
         if ( WifiContents::where( 'contentid' , '=' , $contentid )->update ( $data ) )
         {
            return $contentid;
         } else {
            return false;
         ...
         if ( $contentid = WifiContents::insertGetId ( $data ) )
         {
            return $contentid;
         } else {
            return false;
   ...
   public static function delWifiContentsInfo( $condition = [] , $saveFieldData = [] , $isDelete = false )
   {
      ...
      if( true == $isDelete )
      {
         $isStatus = WifiContents::where($condition)->delete();
      ...
      if( false == $isDelete )
      {
         ...
         if ( WifiContents::where ( $condition )->update ( $saveData ) )
         {
            return true;
         } else {
            return false;
   ...
   public static function getWifiContentsOneInfo ( $condition = [] , $orderArr = [] , $fieldsArr = [] , $joinArr = [] )
   {
      ...
      if($joinCount == 1) // 一张表
      {
         return WifiContents::where ( $condition )
      ...
      return WifiContents::where ( $condition )->orderBy ( $orderArr[0],$orderArr[1] )->first ( $fieldsArr );
   }
   ...
   public static function getWifiContentsListInfo( $condition = [] , $orderArr = [] , $pageArr = [] , $fieldsArr = [] , $joinArr = [] )
   {
      ...
      return WifiContents::where ( $condition )->orderBy ( $orderArr[0],$orderArr[1] )
              ->paginate ( $pageArr[ 'limit' ] , $fieldsArr , 'page' , $pageArr[ 'page' ] );
   }
    ...
    * return Int
    */
   public static function getWifiContentsStatistics ( $condition = [] , $mode = 'count' , $field = null )
   {
      // 条件/ 排序必须唯一,必传参数
	  ...
	  if ( $mode == 'count' ) return WifiContents::where ( $condition )->count ();
      if ( $mode == 'max' ) return WifiContents::where ( $condition )->max ( $field );
      if ( $mode == 'min' ) return WifiContents::where ( $condition )->min ( $field );
      if ( $mode == 'avg' ) return (float)WifiContents::where ( $condition )->avg ( $field );
      if ( $mode == 'sum' ) return (float)WifiContents::where ( $condition )->sum ( $field );
      return 0;
   }
}
