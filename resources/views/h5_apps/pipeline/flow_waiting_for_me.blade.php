        ...
        <div class="pipeline-user-flow-box" @click="viewMyApplication({{ $item->userFlow }})">
            <el-card shadow="hover" class="pb-3">
                <div style="display: flex;align-items: center;">
                    <img src="@if(isset($item->userFlow->user->profile->avatar)){{ $item->userFlow->user->profile->avatar }}@endif" style="border-radius: 50%;width: 40px;height: 40px;">
                    <h3 style="margin-left: 20px;flex: 4;">
                        <p style="line-height: 0;font-weight:400;color:#333333;">{{ $item->userFlow->user->name }}的{{ $item->flow->name }}申请</p>
                        <p style="font-size: 13px;color: #ABABAB;margin: 0">类型：{{ $item->flow->name }}</p>
                        <time style="font-size: 13px;color: #ABABAB;">{{ substr($item->userFlow->created_at, 0, 16) }}</time>
                    </h3>
                </div>
            </el-card>
        </div>
